...
export type MantleOpportunity = {
  ...
  walletClusterSignal: number;
  socialConsensus: number;
  autonomyLevel: "assistive" | "supervised" | "guarded";
...
export const mantleOpportunities: MantleOpportunity[] = [
  {
    ...
    sharpeEstimate: 1.62,
    healthFactor: 2.44,
    liquidationBufferPct: 36,
    walletClusterSignal: 82,
    socialConsensus: 74,
    ...
    runbook: [
      "Check mETH redemption queue and Mantle LSP liquidity depth",
      "Simulate staged allocation in 20% slices before approval",
      "Record final operator-approved payload in the decision registry",
    ],
  ...
  },
  {
    ...
    sharpeEstimate: 1.08,
    healthFactor: 1.68,
    liquidationBufferPct: 18,
    walletClusterSignal: 61,
    socialConsensus: 57,
    ...
    runbook: [
      "Read bin depth and slippage before any range movement",
      "Cap rebalance size when volatility exceeds 700 bps",
      "Escalate if fee velocity and wallet-cluster momentum disagree",
    ],
  ...
  },
  {
    ...
    sharpeEstimate: 1.84,
    healthFactor: 3.12,
    liquidationBufferPct: 48,
    walletClusterSignal: 68,
    socialConsensus: 71,
    autonomyLevel: "guarded",
    auditStatus: "verified",
    ...
    runbook: [
      "Confirm defensive allocation target against portfolio drawdown",
      "Check issuer and liquidity assumptions before increasing size",
      "Create evidence hash for the RWA allocation rationale",
    ],
  ...
  },
  {
    ...
    sharpeEstimate: 0.91,
    healthFactor: 2.88,
    liquidationBufferPct: 42,
    walletClusterSignal: 54,
    socialConsensus: 49,
    ...
    runbook: [
      "Compare Agni route against alternative stable pools",
      "Route only defensive capital while yield remains below 3%",
      "Keep action assistive until stable routing data is fully verified",
    ],
